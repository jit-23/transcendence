...
export default function Canvas() {
	...
	useEffect(() => {
		const handleKeyDown = (event: KeyboardEvent) => {
			...
			if (shouldUndo) {
				...
				rotationSessionRef.current = null;
				markDirty();
				emitCanvasEvent(chatSocketRef.current, canvasId, "canvas-undo");
				return;
			}
			...
			rotationSessionRef.current = null;
			markDirty();
			const restoredShape = nextState[nextState.length - 1];
			if (restoredShape) {
				emitCanvasEvent(chatSocketRef.current, canvasId, "canvas-redo", { shape: restoredShape });
			}
		};
  ...
  return (
		<div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
			...
			<div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) 320px", gap: "10px", alignItems: "stretch", minHeight: 0 }}>
					<div ref={canvasHostRef} style={{ minHeight: "320px", border: "1px solid var(--border)", borderRadius: 8, background: "var(--surface)" }} />
					<div style={{ minHeight: 0, height: "100%", display: "flex" }}>
					<CanvasChatSidebar
