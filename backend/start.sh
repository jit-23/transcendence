...
# Generate self-signed SSL certificates if they do not exist, do not match,
# or do not include SAN entries required by modern browsers.
needs_regen=0
if [ ! -f "$CERT_PATH" ] || [ ! -f "$KEY_PATH" ]; then
  ...
  cert_san=$(openssl x509 -in "$CERT_PATH" -noout -ext subjectAltName 2>/dev/null || true)
